...
export function sumPublishedPoints(questions: Question[]): number {
    return sumPoints(
        questions.filter((question: Question) => question.published),
    );
}
...
export function sameType(questions: Question[]): boolean {
    ...
    const firstVal = questions[0].type;
    return questions.every((question) => question.type == firstVal);
}
...
export function duplicateQuestionInArray(
    ...
    const dupe = duplicateQuestion(newId, questions[foundTarget]);
    return [
        ...questions.slice(0, foundTarget + 1),
        dupe,
        ...questions.slice(foundTarget + 1),
    ];
}
